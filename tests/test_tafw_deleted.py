...
be run by hand against a real Dayforce tenant:
    ...
    python tests/test_tafw_deleted.py

It uses the tenant in ``conf/settings.dev.yaml`` and the credentials in the OS
keyring (service ``dayforce-tafw``), and pulls CANCELED TimeAwayFromWork
...
XREF_CODES = ["H5JN767"]
STATUS = DayforceClient.STATUS_CANCELED
...
def main() -> int:
    ...
    log = logging.getLogger("dayforce-tafw-cancelled-test")
    ...
    print("=" * 68)
    print("Dayforce TAFW (CANCELED) test")
    print("-" * 68)
